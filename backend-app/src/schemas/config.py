...
class AppSettings(BaseSettings):
    ...
    def load_graph_config(self, graph_name: str) -> dict:
        ...
        try:
            ...
            _logger.debug(
                f"Successfully loaded and parsed graph config: {config_file_name}"
            )
            return config_data
        except json.JSONDecodeError as e:
            _logger.error(
                f"JSONDecodeError for graph config {config_file_name}: {e}",
                exc_info=True,
            )
            raise
    ...
    MEMORY_MANAGER_CHAT_HISTORY_PREFIX: str = Field(
        default="chat_history",
        description="Prefix for chat history keys in memory manager",
    )

    PRIMARY_LLM_PROVIDER: str = Field(
        ..., description="기본 LLM 제공자 (openai, anthropic 등)"
    )
    FALLBACK_LLM_PROVIDER: Optional[str] = Field(
        None, description="폴백 LLM 제공자 (선택 사항)"
    )
    LLM_PROVIDERS: Dict[str, LLMProviderSettings] = Field(
        ..., description="LLM 제공자별 설정"
    )
    ...
    WEBSOCKET_KEEP_ALIVE_INTERVAL: int = Field(
    ...
    def validate_settings(self) -> List[str]:
        ...
        if self.PRIMARY_LLM_PROVIDER not in self.LLM_PROVIDERS:
            warnings.append(
                f"PRIMARY_LLM_PROVIDER '{self.PRIMARY_LLM_PROVIDER}' not in LLM_PROVIDERS."
            )
        if (
            self.FALLBACK_LLM_PROVIDER
            and self.FALLBACK_LLM_PROVIDER not in self.LLM_PROVIDERS
        ):
            warnings.append(
                f"FALLBACK_LLM_PROVIDER '{self.FALLBACK_LLM_PROVIDER}' not in LLM_PROVIDERS."
            )

        if self.REDIS_URL and (
            self.REDIS_HOST == "localhost"
            or self.REDIS_PORT == 6379
            or self.REDIS_DB == 0
        ):
            try:
                ...
                if (
                    ...
                    and parsed_url.path[1:].isdigit()
                    and self.REDIS_DB == 0
                ):
                    self.REDIS_DB = int(parsed_url.path[1:])
            except Exception as parse_err:
                warnings.append(
                    f"Could not parse REDIS_HOST/PORT/DB from REDIS_URL: {parse_err}"
                )
