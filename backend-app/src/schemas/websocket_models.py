...
class WebSocketMessageBase(BaseModel):
    ...
    timestamp: datetime.datetime = Field(
        ...
        description="메시지 생성 타임스탬프 (UTC 권장)",
    )
...
class StatusUpdateMessage(WebSocketMessageBase):
    ...
    current_node: Optional[str] = Field(
        None, description="현재 실행 중이거나 완료된 노드 ID"
    )
    next_node: Optional[str] = Field(
        None, description="다음에 실행될 것으로 예상되는 노드 ID"
    )
...
class FinalResultMessage(WebSocketMessageBase):
    ...
    error_message: Optional[str] = Field(
        None, description="오류 발생 시 최종 오류 메시지"
    )

    metadata: Optional[Dict[str, Any]] = Field(
        default_factory=dict, description="최종 결과 관련 추가 메타데이터"
    )
