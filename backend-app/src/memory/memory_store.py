...
async def _redis_save_state(key: str, value: Any, ttl: Optional[int] = None) -> bool:
    ...
    try:
        ...
        logger.debug(
            f"[RedisStore] Saving key '{key}'. Size: {data_size_bytes} bytes, TTL: {ttl}s"
        )
        ...
        if not success:
            logger.warning(
                f"[RedisStore] Failed to save key '{key}' (operation returned false)."
            )
        return bool(success)
...
async def _redis_get_history(key_prefix: str, limit: Optional[int] = None) -> List[Any]:
    ...
    try:
        redis = await get_redis_async_connection()
        logger.debug(
            f"[RedisStore] Getting history for prefix '{key_prefix}', limit={limit}"
        )
        ...
        logger.debug(
            f"[RedisStore] SCAN operation took {scan_duration:.4f}s, found {len(keys_bytes)} keys for prefix '{key_prefix}'."
        )
        ...
        def sort_key_func(key_b: bytes) -> float:
            try:
                ...
                logger.warning(
                    f"Could not parse timestamp from key: {key_b.decode(errors='ignore')}"
                )
                return 0.0
        ...
        logger.debug(
            f"[RedisStore] MGET operation took {mget_duration:.4f}s for {len(keys_to_fetch)} keys."
        )
        ...
        for i, data in enumerate(values_bytes):
            if data:
                try:
                    deserialized_item = deserialize(
                        data, format=SerializationFormat.MSGPACK, cls=Any
                    )
                    history.append(deserialized_item)
                except Exception as deser_err:
                    key_str = keys_to_fetch[i].decode(errors="ignore")
                    logger.warning(
                        f"[RedisStore] Failed to deserialize history item for key '{key_str}': {deser_err}"
                    )
            else:
                key_str = keys_to_fetch[i].decode(errors="ignore")
                logger.warning(
                    f"[RedisStore] MGET returned None for key '{key_str}', possibly expired or deleted."
                )

        logger.info(
            f"[RedisStore] Retrieved {len(history)} history items for prefix '{key_prefix}'."
        )
        return history
        ...
        error = convert_exception(
            e,
            ErrorCode.REDIS_OPERATION_ERROR,
            f"Failed to get history for prefix '{key_prefix}' from Redis",
        )
        raise error
...
def _initialize_store_backend():
    ...
    if memory_type == "redis":
        ...
        raise ValueError(
            f"Unsupported MEMORY_TYPE configured: '{memory_type}'. Supported types: 'redis'."
        )
...
async def save_state(key: str, value: Any, ttl: Optional[int] = None) -> bool:
    ...
    if not _backend_initialized:
        _initialize_store_backend()
    func = _backend_functions["save_state"]
    if not func:
        raise RuntimeError("Memory store 'save_state' function not initialized.")
    ...
    try:
        ...
        raise convert_exception(
            e, ErrorCode.MEMORY_STORAGE_ERROR, f"Failed to save state for key '{key}'"
        )
...
async def load_state(key: str, default: Any = None) -> Any:
    ...
    if not func:
        raise RuntimeError("Memory store 'load_state' function not initialized.")
    try:
        ...
        raise convert_exception(
            e, ErrorCode.MEMORY_RETRIEVAL_ERROR, f"Failed to load state for key '{key}'"
        )
...
async def delete_state(key: str) -> bool:
    ...
    if not func:
        raise RuntimeError("Memory store 'delete_state' function not initialized.")
    try:
        ...
        raise convert_exception(
            e, ErrorCode.MEMORY_STORAGE_ERROR, f"Failed to delete state for key '{key}'"
        )
...
async def exists(key: str) -> bool:
    ...
    if not func:
        raise RuntimeError("Memory store 'exists' function not initialized.")
    try:
        return await func(key)
    except MemoryError:
        raise
...
async def get_history(key_prefix: str, limit: Optional[int] = None) -> List[Any]:
    ...
    if not func:
        raise RuntimeError("Memory store 'get_history' function not initialized.")
    try:
        ...
        raise convert_exception(
            e,
            ErrorCode.MEMORY_RETRIEVAL_ERROR,
            f"Failed to get history for prefix '{key_prefix}'",
        )
