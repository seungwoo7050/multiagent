...
def _json_encoder_hook(obj: Any) -> Any:
  ...
  raise TypeError(
    f"Object of type {type(obj)} is not directly serializable to JSON by this hook."
  )
...
def _msgpack_encoder_hook(obj: Any) -> Any:
  ...
  if dataclasses.is_dataclass(obj):
    return dataclasses.asdict(obj)
  raise TypeError(
    f"Object of type {type(obj)} is not directly serializable to msgpack by this hook."
  )
...
def serialize(
  ...
  try:
    if format == SerializationFormat.MSGPACK:
      ...
      if pretty:
        return json.dumps(
          data, default=_json_encoder_hook, ensure_ascii=False, indent=2
        ).encode("utf-8")
      else:
...
def deserialize(
  data: bytes, format: SerializationFormat, cls: Optional[Type[T]] = None
) -> T:
  ...
  try:
    if format == SerializationFormat.PICKLE:
      ...
      if (
        ...
        and "bytes" in result
        and isinstance(result["bytes"], str)
      ):
        try:
...
def serialize_to_json(data: Any, pretty: bool = False) -> str:
  ...
  try:
    serialized_bytes = serialize(
      data, format=SerializationFormat.JSON, pretty=pretty
    )
...
def deserialize_from_json(data: str, cls: Optional[Type[T]] = None) -> T:
  ...
  try:
    result = deserialize(
      data.encode("utf-8"), format=SerializationFormat.JSON, cls=cls
    )
