...
class GenericLLMNode:
            ...
            if key == self.history_prompt_key and self.history_key_prefix:                                  
                ...
                if key == 'available_tools':
                    ...
                    value = "\n".join(
                        ...
                        for c in history_list if isinstance(c, dict)           
    ...
    def _parse_llm_response(self, response_str: str) -> Optional[ParsedLLMResponse]:
        ...
        try:
            ...
            if isinstance(parsed_json, dict) and 'action' in parsed_json and 'action_input' in parsed_json:
                 ...
                 return ParsedLLMResponse(action=str(action_val), action_input=action_input_val)                   
            else:
            ...
            action_input_match = re.search(r"Action Input:\s*([\s\S]+)", response_str, re.IGNORECASE | re.DOTALL)                       
    ...
    async def __call__(self, state: AgentGraphState, config: Optional[RunnableConfig] = None) -> Dict[str, Any]:
        ...
        with tracer.start_as_current_span(
            ...
            current_dynamic_data_for_loop.setdefault('scratchpad', state.scratchpad or "")                          
            current_dynamic_data_for_loop.setdefault('tool_call_history', list(state.tool_call_history or []))                                 
            ...
            for i in range(self.max_react_iterations):
                ...
                try:
                    llm_params_for_call = {}
                    if self.temperature is not None: llm_params_for_call['temperature'] = self.temperature
                    llm_params_for_call['max_tokens'] = self.max_tokens if self.max_tokens else 1000                    
                ...
                if normalized_action == "final_answer" or normalized_action == "finish":
                    ...
                    if not tool_name_from_action or not isinstance(tool_name_from_action, str):
                        ...
                        try:
                            ...
                            tool_result = await tool_instance.ainvoke(tool_args_from_action)                           
                    ...
                    if self.allowed_tools is not None and action not in self.allowed_tools:
                        ...
                        tool_args_direct = action_input if isinstance(action_input, dict) else {}                           
            ...
            if current_error_message:
                ...
                current_node_span.set_status(trace.Status(trace.StatusCode.OK, description="Max iterations reached"))                
