...
class SearchStrategyNode:
    ...
    def _select_best_thought(self, thoughts: List[Thought]) -> Optional[Thought]:
        ...
        scored_thoughts = [
            t
            for t in thoughts
            if t.status == "evaluated" and t.evaluation_score is not None
        ]
    ...
    async def __call__(
        ...
        with tracer.start_as_current_span(
            ...
            logger.info(
                f"SearchStrategyNode '{self.node_id}' execution started. Task ID: {state.task_id}, Depth: {state.search_depth}"
            )
            await self.notification_service.broadcast_to_task(
                ...
                StatusUpdateMessage(
                    ...
                    detail=f"Node '{self.node_id}' (Search Strategy) started.",
            ...
            if not evaluated_thoughts:
                logger.warning(
                    f"Node '{self.node_id}' (Task: {state.task_id}): No evaluated thoughts with scores found."
                )
                await self.notification_service.broadcast_to_task(
                    state.task_id,
                    StatusUpdateMessage(
                        task_id=state.task_id,
                        status="node_error",
                        detail=f"Node '{self.node_id}': No evaluated thoughts to process.",
                        current_node=self.node_id,
                    ),
                )

                final_answer_content = (
                    "No evaluated thoughts were found to solve this problem."
                )
            ...
            best_across_all = self._select_best_thought(state.thoughts)
            new_global_best_thought_id = (
                best_across_all.id if best_across_all else state.current_best_thought_id
            )

            global_best_thought = (
                state.get_thought_by_id(state.current_best_thought_id)
            ...
            if global_best_thought is None or (
                best_across_all
                and best_across_all.evaluation_score is not None
                and (
                    global_best_thought.evaluation_score is None
                    or best_across_all.evaluation_score
                    > global_best_thought.evaluation_score
                )
            ):
                new_global_best_thought_id = best_across_all.id
                logger.info(
                    f"Node '{self.node_id}' (Task: {state.task_id}): New global best thought ID: {new_global_best_thought_id} (Score: {best_across_all.evaluation_score})"
                )
            elif global_best_thought:
                logger.info(
                    f"Node '{self.node_id}' (Task: {state.task_id}): Global best thought remains: {global_best_thought.id} (Score: {global_best_thought.evaluation_score})"
                )
            ...
            if (
                isinstance(state.dynamic_data, dict)
                and "recursion_limit" in state.dynamic_data
            ):
                recursion_limit = state.dynamic_data["recursion_limit"]
            elif (
                isinstance(state.metadata, dict) and "recursion_limit" in state.metadata
            ):
                recursion_limit = state.metadata["recursion_limit"]

            if (
                current_round_best_thought.evaluation_score is not None
                and current_round_best_thought.evaluation_score >= 0.85
            ):
                logger.warning(
                    f"Node '{self.node_id}' (Task: {state.task_id}): Very high score ({current_round_best_thought.evaluation_score}) detected. Forcing termination despite depth."
                )
            ...
            if not should_terminate and isinstance(state.dynamic_data, dict):
                ...
                if best_across_all and best_across_all.evaluation_score is not None:
                    if best_across_all.evaluation_score <= prev_best_score:
                        ...
                        no_improvement_count = 0
                        state.dynamic_data["prev_best_score"] = (
                            best_across_all.evaluation_score
                        )
                state.dynamic_data["no_improvement_count"] = no_improvement_count

            if (
                not should_terminate
                and no_improvement_count >= 3
                and state.search_depth >= 2
            ):
                logger.warning(
                    f"Node '{self.node_id}' (Task: {state.task_id}): No score improvement for 3 iterations. Forcing termination."
                )
                should_terminate = True
                final_thought_to_use = best_across_all or current_round_best_thought
                final_answer_content = (
                    final_thought_to_use.content
                    if final_thought_to_use
                    else "Search stopped due to no progress."
                )
            ...
            if (
                not should_terminate
                and state.search_depth < self.min_depth_before_finish
            ):
                logger.info(
                    f"Node '{self.node_id}' (Task: {state.task_id}): Continuing search as minimum depth {self.min_depth_before_finish} not yet reached."
                )
                ...
                logger.warning(
                    f"Node '{self.node_id}' (Task: {state.task_id}): Recursion limit ({recursion_limit}) nearly reached. Forcing termination."
                )
                should_terminate = True
                final_thought_to_use = best_across_all or current_round_best_thought
                final_answer_content = (
                    final_thought_to_use.content
                    if final_thought_to_use
                    else "Search stopped due to recursion limit."
                )
                ...
                logger.info(
                    f"Node '{self.node_id}' (Task: {state.task_id}): Max search depth ({state.max_search_depth}) reached."
                )
                should_terminate = True
                final_thought_to_use = best_across_all or current_round_best_thought
                final_answer_content = (
                    final_thought_to_use.content
                    if final_thought_to_use
                    else "Reached max depth, no definitive answer."
                )
                ...
                not should_terminate
                and current_round_best_thought.evaluation_score is not None
                and current_round_best_thought.evaluation_score
                >= self.score_threshold_to_finish
            ):
                logger.info(
                    f"Node '{self.node_id}' (Task: {state.task_id}): High-confidence thought found (Score: {current_round_best_thought.evaluation_score}). Finalizing."
                )
                should_terminate = True
                final_answer_content = current_round_best_thought.content
                ...
                and best_across_all
                and best_across_all.evaluation_score is not None
                and best_across_all.evaluation_score
                >= (self.score_threshold_to_finish * 0.85)
                and state.search_depth >= 3
            ):
                logger.info(
                    f"Node '{self.node_id}' (Task: {state.task_id}): Decent solution found after sufficient exploration (Score: {best_across_all.evaluation_score}). Finalizing."
                )
                should_terminate = True
                final_answer_content = best_across_all.content
                ...
                and (state.search_depth >= 2)
                and all(
                    t.evaluation_score is not None
                    and t.evaluation_score < self.min_score_to_continue
                    for t in top_thoughts_for_expansion
                )
            ):
                logger.warning(
                    f"Node '{self.node_id}' (Task: {state.task_id}): All top {self.beam_width} thoughts below threshold. Stopping."
                )
                should_terminate = True
                final_thought_to_use = best_across_all or current_round_best_thought
                ...
                logger.info(
                    f"Node '{self.node_id}' (Task: {state.task_id}): Proceeding to next search depth ({next_search_depth}). Best thought to expand based on: {new_global_best_thought_id}"
                )
                strategy_decision = "continue_search"
                next_action = "continue"
            ...
            if should_terminate:
                ...
                if state.original_input and isinstance(final_answer, str):
                    if not final_answer.startswith(
                        "Based on"
                    ) and not final_answer.startswith("Regarding"):
                        final_answer = f"Based on your request: '{state.original_input}', here is my solution:\n\n{final_answer}"

                update_payload["search_depth"] = (
                    state.search_depth
                    if strategy_decision == "finish_high_score"
                    else next_search_depth
                )
                update_payload["error_message"] = None
                update_payload["final_answer"] = final_answer

                update_payload["dynamic_data"] = (
            ...
            await self.notification_service.broadcast_to_task(
                ...
                IntermediateResultMessage(
                    ...
                    data={
                        ...
                        "current_best_thought_id": new_global_best_thought_id,
                        "current_best_score": state.get_thought_by_id(
                            new_global_best_thought_id
                        ).evaluation_score
                        if new_global_best_thought_id
                        and state.get_thought_by_id(new_global_best_thought_id)
                        else None,
                        "next_depth": next_search_depth
                        if not should_terminate
                        else state.search_depth,
                        "final_answer_preview": (
                            final_answer_content[:100] + "..."
