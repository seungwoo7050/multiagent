...
class TaskComplexityRouterNode:
  ...
  def __init__(
    ...
    self.prompt_template_path = os.path.join(
      settings.PROMPT_TEMPLATE_DIR, prompt_template_path
    )
    with open(self.prompt_template_path, "r") as f:
      self.prompt_template = f.read()

    logger.info(
      f"TaskComplexityRouterNode '{self.node_id}' initialized. Threshold: {self.complexity_threshold}, Prompt: '{prompt_template_path}'"
    )
  ...
  async def __call__(
    ...
    with tracer.start_as_current_span(
      ...
      logger.info(
        f"TaskComplexityRouterNode '{self.node_id}' execution started. Task ID: {state.task_id}"
      )
      ...
      try:
        ...
        for line in lines:
          ...
          if line.startswith("complexity score:"):
            try:
              ...
              logger.warning(
                f"Node '{self.node_id}': Failed to parse complexity score from: {line}"
              )
        ...
        if complexity_score is not None and "decision:" not in response.lower():
          next_action = (
            ...
            else "process_simple_task"
          )

        logger.info(
          f"Node '{self.node_id}' (Task: {state.task_id}): Routing decision: {next_action} (Score: {complexity_score})"
        )
      ...
      await self.notification_service.broadcast_to_task(
        ...
        StatusUpdateMessage(
          ...
          if next_action == "process_complex_task"
