...
class StateEvaluatorNode:
  ...
  def _load_prompt_template_if_path_exists(self) -> Optional[str]:
    ...
    try:
      ...
      logger.warning(
        f"Prompt template file not found for StateEvaluatorNode '{self.node_id}': {full_path}. Using default internal prompt."
      )
      return None
  ...
  def _construct_evaluation_prompt(
    self, state: AgentGraphState, thought_content: str, parent_thought_content: str
  ) -> str:
    if self.prompt_template_str:
      ...
      try:
        ...
        logger.error(
          f"Missing key for prompt template in StateEvaluatorNode '{self.node_id}': {ke}. Falling back to default internal prompt."
        )
      except Exception as e:
        logger.error(
          f"Error formatting prompt template in StateEvaluatorNode '{self.node_id}': {e}. Falling back to default internal prompt."
        )
  ...
  def _parse_evaluation_response(
    ...
    logger.warning(
      f"Could not parse score from evaluation response. Using default score {self.default_score}. Response: {response_str[:100]}..."
    )
    return (
      self.default_score,
      f"Score parsing failed. Original response: {response_str[:200]}...",
    )
  ...
  async def __call__(
    ...
    with tracer.start_as_current_span(
      ...
      logger.info(
        f"StateEvaluatorNode '{self.node_id}' execution started. Task ID: {state.task_id}"
      )
      await self.notification_service.broadcast_to_task(
        ...
        StatusUpdateMessage(
          ...
          detail=f"Node '{self.node_id}' (State Evaluator) started.",
      ...
      if not state.current_thoughts_to_evaluate:
        logger.info(
          f"Node '{self.node_id}' (Task: {state.task_id}): No thoughts to evaluate."
        )
        await self.notification_service.broadcast_to_task(
          state.task_id,
          StatusUpdateMessage(
            ...
            detail=f"Node '{self.node_id}' finished: No thoughts to evaluate.",
            current_node=self.node_id,
            next_node="search_strategy" if state.thoughts else None,
          ),
        )
      ...
      for thought_id_to_eval in state.current_thoughts_to_evaluate:
        ...
        if not thought_to_eval:
          logger.warning(
            f"Node '{self.node_id}' (Task: {state.task_id}): Thought with ID '{thought_id_to_eval}' not found. Skipping."
          )
          continue
        ...
        try:
          eval_prompt = self._construct_evaluation_prompt(
            state, thought_to_eval.content, parent_thought_content
          )
          logger.debug(
            f"Node '{self.node_id}' (Task: {state.task_id}): Evaluation prompt for thought '{thought_to_eval.id}'."
          )
          ...
          logger.debug(
            f"Node '{self.node_id}' (Task: {state.task_id}): Evaluation response for '{thought_to_eval.id}': {eval_response_str[:100]}..."
          )
          ...
          updated_metadata = (
            thought_to_eval.metadata.copy()
            if thought_to_eval.metadata
            else {}
          )
          ...
          evaluations_summary_for_log.append(
            f"Thought '{thought_to_eval.id}': Score={score:.2f}"
          )
          ...
          logger.error(
            f"Node '{self.node_id}' (Task: {state.task_id}): Error evaluating thought '{thought_id_to_eval}': {e}",
            exc_info=True,
          )
          ...
          updated_metadata["eval_reasoning"] = (
            f"Evaluation failed, using default score {self.default_score}. Error: {str(e)}"
          )
          ...
          fallback_thought_version = Thought(
            ...
            parent_id=thought_to_eval.parent_id,
            content=thought_to_eval.content,
            evaluation_score=self.default_score,
            status="evaluated",
            metadata=updated_metadata,
          )
          ...
          any_evaluation_done = True
          evaluations_summary_for_log.append(
            f"Thought '{thought_id_to_eval}': Score={self.default_score:.2f} (fallback)"
          )

          error_message = (
            error_message or ""
          ) + f"Error evaluating {thought_id_to_eval}: {str(e)}; "
          await self.notification_service.broadcast_to_task(
            state.task_id,
            IntermediateResultMessage(
              task_id=state.task_id,
              ...
              data={
                "thought_id": thought_id_to_eval,
                "fallback_score": self.default_score,
                "error": str(e),
              },
            ),
          )
      ...
      log_summary_str = (
        ", ".join(evaluations_summary_for_log)
        if evaluations_summary_for_log
        else "No new evaluations."
      )
      logger.info(
        f"Node '{self.node_id}' (Task: {state.task_id}): Evaluations completed. Summary: {log_summary_str}"
      )
      ...
      if any_evaluation_done:
        update_payload["last_llm_output"] = (
          f"Evaluations completed for thoughts: {', '.join(state.current_thoughts_to_evaluate)}. Summary: {log_summary_str}"
        )
      if error_message:
