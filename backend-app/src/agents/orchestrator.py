...
class Orchestrator:
    ...
    def _load_graph_config_from_file(self, config_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"Graph configuration validation failed for '{config_name}': {ve.errors()}", exc_info=True)                        
    ...
    def _get_conditional_router_func(
        ...
        def router(state: Union[AgentGraphState, Dict[str, Any]]) -> str:
            ...
            state_for_log = str(state)[:200] + "..." if len(str(state)) > 200 else str(state)                
        ...
        for edge_config_model in cfg.edges:                                         
            try:
                if edge_config_model.type == "standard":
                    ...
                    default_target = END if cond_edge_cfg.default_target == "__end__" else (cond_edge_cfg.default_target or END)                               
        ...
        with tracer.start_as_current_span(
            ...
            if conversation_id:
                try:
                    ...
                    summary_storage_key = self._get_summary_storage_key(conversation_id)                    
            ...
            initial_agent_state_obj = AgentGraphState(
                ...
                original_input=original_input,
                metadata=initial_metadata or {},
                max_search_depth=(initial_metadata or {}).get('max_search_depth', settings.TOT_MAX_DEPTH if hasattr(settings, 'TOT_MAX_DEPTH') else 5),                
                dynamic_data={}                 
            )
            ...
            logger.debug(f"Orchestrator: Initial state dict for workflow '{graph_config_name}' (task {task_id}): {str(initial_state_dict)[:500]}...")           
                ...
                if new_generated_summary and new_generated_summary != retrieved_summary :                             
                    try:
                        ...
                        await self.memory_manager.save_state(
                            ...
                            ttl=settings.MEMORY_TTL * 7 if settings.MEMORY_TTL else None                        
                        )
