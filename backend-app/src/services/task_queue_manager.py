...
class TaskQueueManager:
  ...
  def _get_queue_position(self, task_id: str) -> Optional[int]:
    ...
    if task_id not in self.task_info or self.task_info[task_id].status != TaskStatus.QUEUED:
      return None
  ...
  async def _execute_workflow(
    ...
    from src.api.dependencies import get_new_orchestrator_dependency, get_memory_manager_dependency
    ...
    try:
      ...
      await memory_manager.save_state(
        context_id=task_id,
        key=state_key,
        value=final_state.model_dump(mode='json') if hasattr(final_state, 'model_dump') else final_state,
