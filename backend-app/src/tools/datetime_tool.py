...
class DateTimeInput(BaseModel):
    ...
    months: Optional[int] = Field(
        None, description="Number of months to add/subtract.", ge=-1200, le=1200
    )
    days: Optional[int] = Field(
        None, description="Number of days to add/subtract.", ge=-36500, le=36500
    )
    hours: Optional[int] = Field(
        None, description="Number of hours to add/subtract.", ge=-876000, le=876000
    )
    ...
    date1: Optional[str] = Field(
        None, description="First date string for 'diff' operation."
    )
    date2: Optional[str] = Field(
        None, description="Second date string for 'diff' operation."
    )
    ...
    month: Optional[int] = Field(
        None, description="Month (1-12) for 'days_in_month' operation.", ge=1, le=12
    )
...
class DateTimeTool(BaseTool):
    ...
    def _run(self, **kwargs: Any) -> str:
        ...
        logger.debug(
            f"DateTimeTool: Processing operation: {operation.value} with timezone: {timezone_str}"
        )
        ...
        try:
            ...
            if operation == DateTimeOperation.CURRENT:
                ...
                result_dict = self._handle_format(
                    kwargs.get("date_string"), kwargs.get("format_string"), tz
                )
            elif operation == DateTimeOperation.ADD:
                result_dict = self._handle_add(
                ...
                result_dict = self._handle_subtract(
                    kwargs.get("date_string"),
                    kwargs.get("years"),
                    kwargs.get("months"),
                    kwargs.get("days"),
                ...
                result_dict = self._handle_diff(
                    kwargs.get("date1"), kwargs.get("date2"), tz
                )
                ...
                result_dict = self._handle_days_in_month(
            ...
            logger.warning(
                f"DateTimeTool: Validation error during '{operation.value}': {validation_err}"
            )
            raise ToolError(
                code=ErrorCode.TOOL_VALIDATION_ERROR,
            ...
            if isinstance(e, ToolError):
                raise e
            logger.exception(
                f"DateTimeTool: Unexpected error during operation '{operation.value}': {str(e)}"
            )
            raise ToolError(
    ...
    async def _arun(self, **kwargs: Any) -> str:
        ...
        try:
            ...
            logger.exception(
                f"DateTimeTool: Unexpected error during async wrapper: {str(e)}"
            )
            raise ToolError(
                code=ErrorCode.TOOL_EXECUTION_ERROR,
    ...
    def _handle_parse(
        self, date_string: Optional[str], tz: zoneinfo.ZoneInfo
    ) -> Dict[str, Any]:
        if not date_string:
        ...
        try:
            ...
            if (
                dt_from_iso.tzinfo is None
                or dt_from_iso.tzinfo.utcoffset(dt_from_iso) is None
            ):
                parsed_dt = dt_from_iso.replace(
        ...
        return {
            ...
            "components": {
                ...
                "timezone_abbr": parsed_dt.strftime("%Z"),
                "utc_offset": parsed_dt.strftime("%z"),
            },
        }
    ...
    def _handle_format(
        ...
        if not date_string:
            ...
            try:
                ...
                dt_to_format = datetime.datetime.fromisoformat(
                    parse_result["iso_format"]
                )
                input_desc = date_string
            except ValueError as parse_err:
                raise ValueError(
                    f"Failed to parse input date string for formatting: {parse_err}"
                ) from parse_err

        effective_format_string = (
            format_string if format_string else "%Y-%m-%d %H:%M:%S %Z"
        )
        try:
            ...
            return {
                "operation": "format",
                "input": input_desc,
                "format_string": effective_format_string,
                "timezone": str(tz),
                "formatted_string": formatted_string,
                "iso_format": dt_to_format.isoformat(),
            }
    ...
    def _handle_add(
        ...
        if not date_string:
            ...
            try:
                ...
                raise ValueError(
                    f"Failed to parse base date string for addition: {parse_err}"
                ) from parse_err
        ...
        try:
            ...
            if years_to_add != 0 or months_to_add != 0:
                ...
                new_year = (
                    result_dt.year + (result_dt.month - 1 + total_months_delta) // 12
                )
            ...
            return {
                "operation": "add",
                "input": input_desc,
                "additions": {k: v for k, v in additions.items() if v is not None},
                "timezone": str(tz),
                "original_iso": dt_base.isoformat(),
                "result_iso": result_dt.isoformat(),
                "result_formatted": result_dt.strftime("%Y-%m-%d %H:%M:%S %Z"),
            }
        except OverflowError as oe:
            raise ValueError(
                f"Date calculation resulted in overflow: {str(oe)}"
            ) from oe
        except Exception as e:
    ...
    def _handle_diff(
        self, date1_str: Optional[str], date2_str: Optional[str], tz: zoneinfo.ZoneInfo
    ) -> Dict[str, Any]:
        if not date1_str or not date2_str:
            raise ValueError(
                "Both date1 and date2 strings are required for 'diff' operation."
            )
        try:
            parse_result1 = self._handle_parse(date1_str, tz)
            parse_result2 = self._handle_parse(date2_str, tz)
            dt1: datetime.datetime = datetime.datetime.fromisoformat(
                parse_result1["iso_format"]
            )
            dt2: datetime.datetime = datetime.datetime.fromisoformat(
                parse_result2["iso_format"]
            )
            ...
            return {
                "operation": "diff",
                "date1_iso": dt1.isoformat(),
                "date2_iso": dt2.isoformat(),
                "timezone": str(tz),
                "total_seconds": diff.total_seconds(),
                "components": {
                    "years": years,
                    ...
                    "total_days_td": diff.days,
                },
            }
        except ValueError as ve:
            raise ValueError(
                f"Failed to parse date strings for diff operation: {ve}"
            ) from ve
        except Exception as e:
    ...
    def _handle_weekday(
        ...
        if not date_string:
            ...
            try:
                ...
                raise ValueError(
                    f"Failed to parse date string for weekday check: {parse_err}"
                ) from parse_err
        ...
        return {
            ...
            "input": input_desc,
            "timezone": str(tz),
            "iso_format": dt_target.isoformat(),
            "weekday": {
                "number_python": weekday_num,
                "number_iso": iso_weekday_num,
                "name": weekday_name,
            },
        }
    ...
    def _handle_is_weekend(
        ...
        if not date_string:
            ...
            try:
                ...
                raise ValueError(
                    f"Failed to parse date string for weekend check: {parse_err}"
                ) from parse_err
        weekday_num: int = dt_target.weekday()
        ...
        return {
            ...
            "iso_format": dt_target.isoformat(),
            "is_weekend": is_weekend,
            "weekday": {"number_python": weekday_num, "name": dt_target.strftime("%A")},
        }
