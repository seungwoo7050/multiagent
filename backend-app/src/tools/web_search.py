...
class WebSearchTool(BaseTool):
    ...
    def _get_search_instance(self) -> DuckDuckGoSearchRun:
        ...
        if self._search_instance is None:
            try:
                ...
                logger.error(
                    f"Failed to initialize DuckDuckGoSearchRun: {e}", exc_info=True
                )
                raise ToolError(
    ...
    def _run(self, query: str) -> str:
        ...
        try:
            ...
            logger.info(
                f"WebSearchTool: Search successful for query '{query}'. Result length: {len(result)}"
            )
            return result
        except Exception as e:
            logger.error(
                f"WebSearchTool: Error during DuckDuckGo search for query '{query}': {e}",
                exc_info=True,
            )
            raise ToolError(
    ...
    async def _arun(self, query: str) -> str:
        ...
        """
        logger.debug(
            f"WebSearchTool: Asynchronously executing search for query: '{query}'"
        )
        try:
            search_tool = self._get_search_instance()
            ...
            logger.info(
                f"WebSearchTool: Async search successful for query '{query}'. Result length: {len(result)}"
            )
            return result
        except Exception as e:
            logger.error(
                f"WebSearchTool: Error during async DuckDuckGo search for query '{query}': {e}",
                exc_info=True,
            )
            raise ToolError(
