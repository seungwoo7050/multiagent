...
G = None
import asyncio as M
from typing import Annotated as D
...
async def U():
    ...
    if F is G:
        async with T:
            if F is G:
                F = L()
                A.info("NotificationService singleton instance created.")
    return F
...
async def V():
    try:
        ...
        if D is G:
            raise B(
                status_code=C.HTTP_503_SERVICE_UNAVAILABLE,
                detail="Memory service is not available.",
            )
        return D
    except I as E:
        A.error(f"Failed to get MemoryManager dependency: {E}", exc_info=H)
        raise B(
...
async def W():
    try:
        ...
        A.error(f"Failed to get LLMClient dependency: {E}", exc_info=H)
        raise B(
            status_code=C.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def X():
    try:
        ...
        A.error(f"Failed to get ToolManager dependency: {E}", exc_info=H)
        raise B(
            status_code=C.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def Y(llm_client, tool_manager, memory_manager, notification_service):
    ...
    try:
        ...
        A.debug("New Orchestrator instance created successfully.")
        return E
    except ValueError as D:
        A.error(f"New Orchestrator dependency error: {D}", exc_info=H)
        raise B(
            status_code=C.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Orchestrator configuration error: {D}",
        )
    except I as F:
        A.error(f"Failed to get New Orchestrator dependency: {F}", exc_info=H)
        raise B(
            status_code=C.HTTP_500_INTERNAL_SERVER_ERROR,
