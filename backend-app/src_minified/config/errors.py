...
class C(Exception):
    def __init__(A, code, message, details=B, original_error=B):
        B = message
        A.code = code
        A.message = B
        A.details = details or {}
        A.original_error = original_error
        super().__init__(B)
    ...
    def to_dict(B):
        ...
        if B.details:
            C["details"] = B.details
        if B.original_error:
            C["original_error"] = J(B.original_error)
        return C

    def log_error(B, logger_instance=B):
        F = "error_details"
        C = logger_instance or L
        D = B.to_dict()
        E = B.code.value if K(B.code, A) else B.code
        if B.original_error:
            C.error(f"{E}: {B.message}", extra={F: D}, exc_info=B.original_error)
        else:
            C.error(f"{E}: {B.message}", extra={F: D})
...
class M(E):
    def __init__(B, message="Validation error", details=B, original_error=B):
        super().__init__(
            A.VALIDATION_ERROR, message, details, original_error, status_code=400
        )
...
class MemoryError(C):
    def __init__(
        A,
        code=A.MEMORY_ERROR,
        message="A memory error occurred",
        details=B,
        original_error=B,
    ):
        super().__init__(code, message, details, original_error)
...
class I(C):
    def __init__(
        A,
        code=A.ORCHESTRATION_ERROR,
        message="An orchestration error occurred",
        details=B,
        original_error=B,
    ):
        super().__init__(code, message, details, original_error)
...
R = {
    A.SYSTEM_ERROR: SystemError,
    A.CONFIG_ERROR: SystemError,
    A.INITIALIZATION_ERROR: SystemError,
    A.SHUTDOWN_ERROR: SystemError,
    A.API_ERROR: E,
    A.VALIDATION_ERROR: M,
    A.AUTHENTICATION_ERROR: E,
    A.AUTHORIZATION_ERROR: E,
    A.RATE_LIMIT_ERROR: E,
    A.ENDPOINT_NOT_FOUND: N,
    A.BAD_REQUEST: E,
    A.TASK_ERROR: F,
    A.TASK_NOT_FOUND: F,
    A.TASK_CREATION_ERROR: F,
    A.TASK_EXECUTION_ERROR: F,
    A.TASK_TIMEOUT: F,
    A.TASK_CANCELED: F,
    A.LLM_ERROR: D,
    A.LLM_API_ERROR: D,
    A.LLM_TIMEOUT: D,
    A.LLM_RATE_LIMIT: D,
    A.LLM_CONTENT_FILTER: D,
    A.LLM_CONTEXT_LIMIT: D,
    A.LLM_TOKEN_LIMIT: D,
    A.LLM_PROVIDER_ERROR: D,
    A.MEMORY_ERROR: MemoryError,
    A.REDIS_CONNECTION_ERROR: MemoryError,
    A.REDIS_OPERATION_ERROR: MemoryError,
    A.MEMORY_RETRIEVAL_ERROR: MemoryError,
    A.MEMORY_STORAGE_ERROR: MemoryError,
    A.VECTOR_DB_ERROR: MemoryError,
    A.AGENT_ERROR: G,
    A.AGENT_NOT_FOUND: G,
    A.AGENT_CREATION_ERROR: G,
    A.AGENT_EXECUTION_ERROR: G,
    A.AGENT_TIMEOUT: G,
    A.TOOL_ERROR: H,
    A.TOOL_NOT_FOUND: H,
    A.TOOL_EXECUTION_ERROR: H,
    A.TOOL_TIMEOUT: H,
    A.TOOL_VALIDATION_ERROR: H,
    A.ORCHESTRATION_ERROR: I,
    A.WORKFLOW_ERROR: I,
    A.DISPATCHER_ERROR: I,
    A.WORKER_ERROR: I,
    A.CIRCUIT_BREAKER_OPEN: I,
    A.CONNECTION_ERROR: ConnectionError,
    A.HTTP_ERROR: ConnectionError,
    A.NETWORK_ERROR: ConnectionError,
    A.TIMEOUT_ERROR: ConnectionError,
}
...
def S(code, message, details=B, original_error=B, **H):
    ...
    if K(B, J):
        try:
            ...
            L.warning(f"Invalid error code string '{B}'. Using SYSTEM_ERROR.")
            B = A.SYSTEM_ERROR
    D = R.get(B, C)
    if D == M:
        return D(message=E, details=F, original_error=G)
    elif D == N:
        Q = H.get(P, I)
        S = H.get(O, I)
        return D(resource_type=S, resource_id=Q, message=E, details=F, original_error=G)
    else:
        return D(code=B, message=E, details=F, original_error=G, **H)
...
def T(exception, default_code=A.SYSTEM_ERROR, default_message=B):
    ...
    if K(A, C):
        return A
    B = default_message or J(A)
    return S(code=default_code, message=B, original_error=A)


U = [
    A.LLM_TIMEOUT,
    A.LLM_RATE_LIMIT,
    A.REDIS_CONNECTION_ERROR,
    A.CONNECTION_ERROR,
    A.HTTP_ERROR,
    A.NETWORK_ERROR,
    A.TIMEOUT_ERROR,
...
V = {
    ...
    A.BAD_REQUEST: 400,
    A.AUTHENTICATION_ERROR: 401,
    A.AUTHORIZATION_ERROR: 403,
    A.ENDPOINT_NOT_FOUND: 404,
    A.TASK_NOT_FOUND: 404,
    A.AGENT_NOT_FOUND: 404,
    A.TOOL_NOT_FOUND: 404,
    A.RATE_LIMIT_ERROR: 429,
    A.LLM_RATE_LIMIT: 429,
    A.LLM_CONTEXT_LIMIT: 400,
    A.LLM_TOKEN_LIMIT: 400,
    A.SYSTEM_ERROR: 500,
    A.CONFIG_ERROR: 500,
    A.INITIALIZATION_ERROR: 500,
    A.SHUTDOWN_ERROR: 500,
    A.API_ERROR: 500,
    A.TASK_ERROR: 500,
    A.TASK_CREATION_ERROR: 500,
    A.TASK_EXECUTION_ERROR: 500,
    A.LLM_ERROR: 502,
    A.LLM_API_ERROR: 502,
    A.LLM_PROVIDER_ERROR: 502,
    A.MEMORY_ERROR: 500,
    A.REDIS_CONNECTION_ERROR: 503,
    A.REDIS_OPERATION_ERROR: 500,
    A.MEMORY_RETRIEVAL_ERROR: 500,
    A.MEMORY_STORAGE_ERROR: 500,
    A.VECTOR_DB_ERROR: 503,
    A.AGENT_ERROR: 500,
    A.AGENT_CREATION_ERROR: 500,
    A.AGENT_EXECUTION_ERROR: 500,
    A.TOOL_ERROR: 500,
    A.TOOL_EXECUTION_ERROR: 500,
    A.TOOL_VALIDATION_ERROR: 400,
    A.ORCHESTRATION_ERROR: 500,
    A.WORKFLOW_ERROR: 500,
    A.DISPATCHER_ERROR: 500,
    A.WORKER_ERROR: 500,
    A.CIRCUIT_BREAKER_OPEN: 503,
    A.CONNECTION_ERROR: 503,
    A.HTTP_ERROR: 502,
    A.NETWORK_ERROR: 504,
    A.TIMEOUT_ERROR: 504,
    A.LLM_TIMEOUT: 504,
