...
class J(B.ABC, H):
    ...
    def get_metadata(A):
        return {"version": A.version, "context_type": A.__class__.__name__}

    model_config = E(arbitrary_types_allowed=C)


class F(J):
    context_id = A(default_factory=I, description="컨텍스트 인스턴스의 고유 식별자")
    timestamp = A(
...
class K(F):
    task_id = A(..., description="연관된 작업의 ID")
    task_type = A(..., description="작업 유형")
    input_data = A(None, description="작업 입력 데이터")
    current_step = A(None, description="현재 진행 중인 단계")
