...
class TaskDivisionNode:
    def __init__(
        ...
        logger.info(
            f"TaskDivisionNode '{self.node_id}' initialized. Max subtasks: {self.max_subtasks}, Min subtasks: {self.min_subtasks}. Prompt: '{self.prompt_template_path if self.prompt_template_path else 'Default internal'}'"
        )
    ...
    def _parse_subtasks(self, response):
        ...
        for line in lines:
            ...
            if line.lower().startswith("subtask #") or line.lower().startswith(
                ...
                if current_subtask and B in current_subtask and A in current_subtask:
                    subtasks.append(current_subtask)
                title_part = line.split(C, 1)[1].strip() if C in line else ""
                current_subtask = {
                    "id": str(uuid.uuid4()),
                    B: title_part,
                    A: "",
                ...
                current_subtask[A] = line.split(C, 1)[1].strip()
            elif current_subtask and A in current_subtask and current_subtask[A]:
                current_subtask[A] += " " + line
            elif current_subtask and not current_subtask[A]:
                if not current_subtask[B]:
                    current_subtask[B] = line
                else:
                    current_subtask[A] = line
        if current_subtask and B in current_subtask and A in current_subtask:
            subtasks.append(current_subtask)
    ...
    async def __call__(self, state, config=_A):
        ...
        with tracer.start_as_current_span(
                ...
                if not subtasks:
                    ...
                    await self.notification_service.broadcast_to_task(
                        ...
                        IntermediateResultMessage(
                            ...
                            data={"subtask_count": len(subtasks), A: subtasks},
                        ),
                    )
            except Exception as e:
                logger.error(
            ...
            if not state.dynamic_data:
                state.dynamic_data = {}
            state.dynamic_data[A] = subtasks
            state.dynamic_data["current_subtask_index"] = 0 if subtasks else _A
            await self.notification_service.broadcast_to_task(
                state.task_id,
                StatusUpdateMessage(
                    task_id=state.task_id,
            ...
            return {
                ...
                "last_llm_output": full_response if "full_response" in locals() else _A,
            }
