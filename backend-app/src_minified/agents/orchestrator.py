...
class Orchestrator:
    def __init__(self, llm_client, tool_manager, memory_manager, notification_service):
        ...
        self.summary_memory_key = _D
        self.summary_prompt_template_str = _A
    ...
    async def _generate_conversation_summary(
        ...
        if current_agent_response is _A or str(current_agent_response).strip() == "":
            logger.debug(
    ...
    def _load_graph_config_from_file(self, config_name):
        ...
        try:
            ...
            if not config_path.exists():
                ...
                if config_path_no_ext.with_suffix(A).exists():
                    config_path = config_path_no_ext.with_suffix(A)
                else:
                    logger.error(
    ...
    def _create_node_instance(self, node_config):
        ...
        try:
            constructor_params = {D: self.llm_client, "node_id": node_id}
            sig_params = inspect.signature(node_class.__init__).parameters
            if A in sig_params and A not in node_params:
                constructor_params[A] = self.tool_manager
                logger.debug(
                    f"Injecting default ToolManager into node '{node_id}' (type: {node_type_str})."
                )
            if B in sig_params and B not in node_params:
                constructor_params[B] = self.memory_manager
                logger.debug(
                    f"Injecting default MemoryManager into node '{node_id}' (type: {node_type_str})."
                )
            if C in sig_params and C not in node_params:
                constructor_params[C] = self.notification_service
                logger.debug(
    ...
    def _get_conditional_router_func(
        self, condition_key, targets_map, default_decision=END
    ):
        def router(state):
            E = "value_is_none"
            D = "value_is_not_none"
            ...
            if is_dict:
                ...
                if condition_key in state_dict:
                    ...
                    key_path = condition_key.split(A)[1:]
                    current_val = state_dict.get(_C)
                ...
                if hasattr(state_obj, condition_key):
                    ...
                    condition_key.startswith(B)
                    and A in condition_key
            ...
            normalized_targets_map = {
                str(k).lower(): v for (k, v) in targets_map.items()
            }
            if str_value_to_check in normalized_targets_map:
                ...
                return decision
            if D in targets_map and value_to_check is not _A:
                decision = targets_map[D]
                logger.debug(
                    f"Router decision based on 'value_is_not_none': '{decision}'"
                )
                return decision
            if E in targets_map and value_to_check is _A:
                decision = targets_map[E]
        ...
        for edge_config_model in cfg.edges:
            try:
                if edge_config_model.type == "standard":
                    ...
                    target_node = (
                        END if std_edge_cfg.target == A else std_edge_cfg.target
                    )
                    ...
                    default_target = (
                        END
                        if cond_edge_cfg.default_target == A
    ...
    async def run_workflow(
        ...
        with tracer.start_as_current_span(
            ...
            conversation_id = _A
            if initial_metadata and isinstance(initial_metadata.get(A), str):
                conversation_id = initial_metadata[A]
                logger.info(
            ...
            if conversation_id:
                try:
                    ...
                    if retrieved_summary:
                        ...
                        current_span.set_attribute(B, _B)
                        ...
                        current_span.set_attribute(B, C)
                except Exception as e:
                    logger.error(
            ...
            try:
                ...
                await self.notification_service.broadcast_to_task(
                    task_id,
                    FinalResultMessage(
                        task_id=task_id, final_answer=_A, error_message=error_detail
                    ),
                )
                current_span.set_status(
                    trace.Status(
            ...
            if retrieved_summary:
                initial_agent_state_obj.dynamic_data[_D] = retrieved_summary
                logger.debug(
            ...
            try:
                ...
                recursion_limit = config_from_json.get(D, max_iterations)
                current_span.set_attribute(
                    "app.workflow.recursion_limit", recursion_limit
                )
                invoke_config = {D: recursion_limit}
                logger.debug(
                    f"Orchestrator: Using recursion_limit={recursion_limit} for workflow '{graph_config_name}' (task {task_id})"
                )
                ...
                if final_state_dict_from_graph and isinstance(
                    ...
                    try:
                        final_state_obj = msgspec.convert(
                            final_state_dict_from_graph, AgentGraphState, strict=C
                        )
                        logger.info(
                        ...
                        final_state_obj = AgentGraphState(
                            ...
                            error_message=error_detail,
                            dynamic_data=final_state_dict_from_graph.get(_C, {}),
                        )
                else:
                    logger.error(
                    ...
                    final_state_obj = AgentGraphState(
                        ...
                        metadata=initial_metadata or {},
                        error_message=error_detail,
                        dynamic_data=initial_state_dict.get(_C, {}),
                    )
                ...
                if new_generated_summary and new_generated_summary != retrieved_summary:
                    try:
                        await self.memory_manager.save_state(
                            ...
                            ttl=settings.MEMORY_TTL * 7 if settings.MEMORY_TTL else _A,
                        )
                        logger.info(
                            f"Orchestrator: Saved new summary for conversation_id '{conversation_id}'. Length: {len(new_generated_summary)}"
                        )
                        current_span.set_attribute("app.summary.saved", _B)
                        current_span.set_attribute(
            ...
            if final_state_obj:
                ...
                await self.notification_service.broadcast_to_task(
                    task_id,
                    FinalResultMessage(
                        task_id=task_id, final_answer=_A, error_message=critical_error
                    ),
                )
                current_span.set_status(
    ...
    def _get_summary_storage_key(self, conversation_id):
