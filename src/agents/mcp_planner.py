...
from src.core.exceptions import AgentExecutionError, LLMError, TaskError
...
class MCPPlannerAgent(BaseAgent):
    ...
    async def process(self, context: CoreAgentContext) -> AgentResult:
        ...
        prompt_data: Dict[str, Any] = {'goal': task_mcp_context.input_data.get('goal', 'No goal provided.'), 'available_tools': self.config.allowed_tools, 'conversation_history': conversation_history}
        try:
        ...
        llm_input_context = LLMInputContext(model=self.config.model or settings.PRIMARY_LLM, messages=[{'role': 'user', 'content': planning_prompt}], parameters={'max_tokens': self.config.parameters.get('max_tokens', 1500), 'temperature': self.config.parameters.get('temperature', 0.5), 'top_p': self.config.parameters.get('top_p', 1.0)}, use_cache=True)
        ...
        try:
            ...
            error_ctx = LLMOutputContext(success=False, error_message=str(e), model_used=llm_input_context.model)
            self.context_manager.update_context(error_ctx)
            raise AgentExecutionError(f'LLM interaction failed: {e}', agent_type=self.config.agent_type, agent_id=self.config.name)
        if not llm_output_context.success or not llm_output_context.result_text:
            ...
            raise AgentExecutionError(f'LLM generation failed: {error_msg}', agent_type=self.config.agent_type, agent_id=self.config.name)
        ...
        try:
            ...
            try:
                ...
                match = re.search('```json\\s*(\\{.*?\\})\\s*```', raw_plan_text, re.DOTALL)
                if match:
            ...
            agent_output: Dict[str, Any] = {'plan': parsed_plan['plan'], 'raw_llm_response': raw_plan_text, 'llm_usage': llm_output_context.usage}
            task_result: Optional[TaskResult] = None
            return AgentResult.success_result(output=agent_output, execution_time=0, task_result=task_result, metadata={'agent_name': self.config.name, 'model_used': llm_output_context.model_used})
        except (json.JSONDecodeError, ValueError) as e:
            logger.error(f'Failed to parse or validate plan JSON: {e}\nRaw response snippet: {raw_plan_text[:500]}', exc_info=True)
            raise AgentExecutionError(f'Failed to process LLM plan output: {e}. Raw Response Snippet: {raw_plan_text[:500]}...', agent_type=self.config.agent_type, agent_id=self.config.name, details={'raw_output': raw_plan_text})
        except Exception as e:
            logger.exception(f'Unexpected error during plan processing: {e}')
            raise AgentExecutionError(f'Unexpected error processing plan: {e}', agent_type=self.config.agent_type, agent_id=self.config.name, original_error=e)
    ...
    async def handle_error(self, error: Exception, context: CoreAgentContext) -> AgentResult:
        ...
        error_details: Dict[str, Any] = {'message': str(error), 'type': type(error).__name__, 'agent_name': self.config.name, 'agent_type': self.config.agent_type}
        if hasattr(error, 'details'):
            error_details['details'] = getattr(error, 'details')
        return AgentResult.error_result(error=error_details, execution_time=0, metadata={'agent_name': self.config.name})