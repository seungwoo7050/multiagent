...
from src.core.mcp.schema import BaseContextSchema, TaskContext, Task
from src.core.mcp.adapters.llm_adapter import LLMInputContext, LLMOutputContext, LLMAdapter
from src.tools.runner import ToolRunner
from src.tools.registry import ToolRegistry, get_registry as get_tool_registry
from src.config.logger import get_logger_with_context, ContextLoggerAdapter
from src.core.exceptions import AgentExecutionError, LLMError, ToolError, TaskError
...
class MCPExecutorAgent(BaseAgent):
    ...
    def __init__(self, config: AgentConfig, tool_registry: Optional[ToolRegistry]=None):
        ...
        self.tool_registry = tool_registry or get_tool_registry('global_tools')
    ...
    async def process(self, context: CoreAgentContext) -> AgentResult:
        ...
        for i in range(self.max_react_iterations):
            ...
            if action_type.lower() == 'finish':
                ...
                try:
                    tool_result: Dict[str, Any] = await self.tool_runner.run_tool(tool=action_type, tool_registry=self.tool_registry, args=action_input, trace_id=context.trace_id)
        ...
        agent_output: Dict[str, Any] = {'final_answer': final_answer, 'scratchpad': scratchpad, 'iterations': executed_steps}
        task_result: Optional[TaskResult] = None
        return AgentResult.success_result(output=agent_output, execution_time=0, task_result=task_result, metadata={'agent_name': self.config.name})
    ...
    async def _generate_thought(self, goal: str, plan: List[Dict[str, Any]], scratchpad: str, executed_steps: int) -> str:
        ...
        llm_input_context = LLMInputContext(model=self.config.model or settings.PRIMARY_LLM, messages=[{'role': 'user', 'content': prompt}], parameters={'max_tokens': 150, 'temperature': 0.5, 'stop_sequences': ['\nAction:']}, use_cache=False)
        try:
    ...
    async def _generate_action(self, goal: str, plan: List[Dict[str, Any]], scratchpad: str, executed_steps: int) -> str:
        ...
        llm_input_context = LLMInputContext(model=self.config.model or settings.PRIMARY_LLM, messages=[{'role': 'user', 'content': prompt}], parameters={'max_tokens': 150, 'temperature': 0.3, 'stop_sequences': ['\nObservation:']}, use_cache=False)
        try:
            llm_output_context = await self.llm_adapter.process_with_mcp(llm_input_context)
            if llm_output_context.success and llm_output_context.result_text:
                ...
                if not re.match('^[a-zA-Z0-9_]+\\s*\\[.*?\\]?\\s*$', action_str):
    ...
    def _parse_action(self, action_str: str) -> Tuple[str, Dict[str, Any]]:
        match = re.match('^\\s*([a-zA-Z0-9_]+)\\s*\\[(.*?)\\]?\\s*$', action_str, re.DOTALL)
        if not match:
