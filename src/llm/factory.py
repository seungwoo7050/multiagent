    ...
    async def create_adapter(self, model_name: str, config: Optional[Dict[str, Any]]=None) -> BaseLLMAdapter:
        ...
        try:
            ...
            if not adapter_instance.initialized:
                ...
                if not initialized:
                    raise LLMError(code=ErrorCode.INITIALIZATION_ERROR, message=f'Failed to initialize created adapter for model {model_name}', model=model_name)
            ...
            raise LLMError(code=ErrorCode.LLM_PROVIDER_ERROR, message=f'Failed to create adapter instance for model {model_name}: {str(e)}', model=model_name, original_error=e)
