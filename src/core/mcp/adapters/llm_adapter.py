...
from src.core.exceptions import SerializationError, LLMError
from src.config.settings import get_settings
from src.config.metrics import track_llm_response, track_llm_error
...
class LLMAdapter(MCPAdapterBase):
    ...
    async def adapt_input(self, context: ContextProtocol, **kwargs: Any) -> Dict[str, Any]:
        ...
        logger.debug(f"Adapting LLMInputContext (ID: {llm_input_context.context_id}) for model '{model}'")
        try:
    ...
    async def process_with_mcp(self, context: ContextProtocol, **kwargs: Any) -> ContextProtocol:
        ...
        try:
            ...
            primary_model_selected, fallback_models_selected = await select_models(requested_model=requested_model, task_context=None)
            ...
            model_actually_used, llm_result_data = await execute_with_fallbacks(primary_model=primary_model_selected, fallback_models=fallback_models_selected, prompt=common_call_args['prompt'], max_tokens=common_call_args.get('max_tokens'), temperature=common_call_args.get('temperature'), top_p=common_call_args.get('top_p'), additional_params=common_call_args.get('additional_params'), timeout=common_call_args.get('timeout'), track_metrics=common_call_args.get('track_metrics', True))
            duration = time.time() - start_time
            context_labels = get_context_labels(context)
            usage = llm_result_data.get('usage', {})
            prompt_tokens = usage.get('prompt_tokens', 0)
            completion_tokens = usage.get('completion_tokens', 0)
            track_llm_response(model=model_actually_used, provider=settings.LLM_MODEL_PROVIDER_MAP.get(model_actually_used, 'unknown'), duration=duration, prompt_tokens=prompt_tokens, completion_tokens=completion_tokens, context_labels=context_labels)
            ...
            track_llm_error(model=model_actually_used, provider=settings.LLM_MODEL_PROVIDER_MAP.get(model_actually_used, 'unknown'), error_type=error_type, context_labels=context_labels)
        except Exception as e:
            ...
            duration = time.time() - start_time
            context_labels = get_context_labels(context)
            track_llm_error(model=model_actually_used, provider=settings.LLM_MODEL_PROVIDER_MAP.get(model_actually_used, 'unknown'), error_type=type(e).__name__, context_labels=context_labels)
        output_context: LLMOutputContext = await self.adapt_output(component_output=component_error if component_error else llm_result_data, original_context=context, model_used=model_actually_used, **kwargs)
