...
class ErrorCode(str, Enum):
    ...
    # Orchestration errors (8000-8999)
...
class BaseError(Exception):
    ...
    def log_error(self, logger_instance=None):
        ...
        if self.original_error:
            logger_to_use.error(f"{self.code.value if isinstance(self.code, ErrorCode) else self.code}: {self.message}", extra={"error_details": error_dict}, exc_info=self.original_error)
        else:
            logger_to_use.error(f"{self.code.value if isinstance(self.code, ErrorCode) else self.code}: {self.message}", extra={"error_details": error_dict})
...
class TaskError(BaseError):
    def __init__(
        ...
        if task_id and details is None:
            details = {"task_id": task_id}
        elif task_id and details is not None:
            details["task_id"] = task_id
...
# Error registry to map error codes to error classes
...
def convert_exception(
    ...
    """Convert any exception to a BaseError instance."""
...
# List of errors that can be retried
RETRYABLE_ERRORS = [
