...
class ToolRunner:
    """Executes tools with proper error handling and metrics tracking."""
    ...
    async def run_tool(
        ...
        """
        Run a tool with proper error handling and retry logic.
        
        Args:
            tool: Tool instance or tool name
            tool_registry: Registry to resolve tool by name
        ...
        TOOL_METRICS['executions'].labels(tool_name=tool_name).inc()
        ...
        try:
            ...
            TOOL_METRICS['errors'].labels(tool_name=tool_name, error_type=str(error.code)).inc()
            
            # Retry if attempts remain
            if retry_count > 0:
                logger.warning(
                    f"Retrying tool '{tool_name}' due to error ({error.code}). Attempts remaining: {retry_count}", 
                ...
                base_delay = 0.2
                max_delay = 2.0
                backoff_time = min(base_delay * (1 + random.random()), max_delay)
    ...
    async def _resolve_tool(self, tool: Union[BaseTool, str], tool_registry: Optional[Any]=None) -> Tuple[BaseTool, str]:
        """
        Resolve a tool from an instance or name.
        
        Args:
            tool: Tool instance or name
            tool_registry: Registry to look up tools by name
            
        Returns:
            Tuple of (tool_instance, tool_name)
            
        Raises:
            ToolError: If tool cannot be resolved
        """
            ...
            if tool_registry is not None and hasattr(tool_registry, 'get_tool'):
                try:
                    ...
                    raise ToolError(
                        ...
                        message=f"Error resolving tool '{tool_name_str}' from registry: {str(e)}", 
                        details={'name': tool_name_str}, 
                        original_error=e, 
        ...
        raise ToolError(
            code=ErrorCode.TOOL_VALIDATION_ERROR, 
            message=error_msg, 
            details={'provided_type': str(type(tool))}
        )
    ...
    def _format_result(self, result: Any, tool_name: str, execution_time: float) -> Dict[str, Any]:
        ...
        if result is None or isinstance(result, (dict, list, str, int, float, bool)):
            ...
            try:
                ...
                logger.warning(
                    f"Could not convert result of type {result_type} to string for tool '{tool_name}': {str_conv_err}"
                )
    ...
    async def _create_tool_task(self, tool_name: str, tool_args: Dict[str, Any], registry: Any) -> asyncio.Task[Any]:
        """Create an asyncio task for tool execution."""
        try:
    ...
    async def run_tools_parallel(
        ...
        Args:
            ...
            registry: Tool registry to resolve tools
        ...
        if tasks:
            ...
            try:
                ...
                results = [
                    {'status': 'error', 'tool_name': name, 'error': f'Wait Error: {str(wait_err)}'} 
            ...
            for task in pending:
                ...
                timeout_error = ToolError(
                    ...
                    message=f"Tool '{tool_name_pending}' timed out after {timeout} seconds", 
